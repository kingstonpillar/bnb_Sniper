...
/* ================= AUTO BUY ================= */
export async function autoBuyToken(tokenMint) {
  if (!(await startCaller())) return null;

  // 1️⃣ Check swap fee BEFORE executing trade
  const feeOk = await swapFeeCheck(tokenMint); // ✅ use tokenMint
  if (!feeOk) {
    console.log(`❌ Swap aborted for ${tokenMint}: fee too high`); // ✅ use tokenMint
    return null; // or throw error to revert
  }

  // ... rest of your code also use tokenMint instead of token
}
