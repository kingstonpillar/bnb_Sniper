...
export async function autoBuyToken(tokenMint) {
  ...
  // --- 1️⃣ Market health check ---
  const isSafe = await marketHealthPass(tokenMint);
  if (!isSafe) {
    console.log(`⛔ Token ${tokenMint} blocked by market health/security checks`);
    return null; // Only gas spent
  }
