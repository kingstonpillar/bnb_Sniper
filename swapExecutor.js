...
function readTradeConfig() {
  ...
  try {
    ...
    const v = data?.tradePerEntry ? Number(data.tradePerEntry) : null;
...
export async function autoBuyToken(tokenMintRaw, pairAddressRaw, { offline = true } = {}) {
  console.log("autoBuyToken entered with:", tokenMintRaw, pairAddressRaw);
  ...
  return await withRpcFailover(async (prov) => {
    ...
    if (!offline) {
      ...
      // Record position: required canonical fields + pairAddress + txHash
