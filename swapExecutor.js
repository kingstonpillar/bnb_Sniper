...
/* ================= AUTO BUY ================= */
/* ================= AUTO BUY ================= */
export async function autoBuyToken(tokenMintRaw, pairAddressRaw) {
  console.log("➡️ autoBuyToken entered with:", tokenMintRaw, pairAddressRaw);

  if (!(await startCaller())) return null;

  // ✅ Checksummed addresses
  const tokenMint = ethers.getAddress(tokenMintRaw);
  const pairAddress = ethers.getAddress(pairAddressRaw);

  // 1️⃣ Check swap fee BEFORE executing trade
  const feeOk = await swapFeeCheck(tokenMint, pairAddress); // Pass pair if swapFeeCheck supports it
  if (!feeOk) {
    console.log(`❌ Swap aborted for ${tokenMint}: fee too high`);
    return null;
  }

  // --- 2️⃣ Continue normal buy if safe ---
  ...
  return await withRpcFailover(async (prov) => {
    ...
    // ✅ Use pairAddress for token path if needed
    const WBNB = ethers.getAddress("0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c");
    const path = [WBNB, tokenMint]; // Still WBNB → token, but pairAddress is available if needed
