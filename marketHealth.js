...
const WBNB = ethers.getAddress(reqEnv("WBNB_ADDRESS")).toLowerCase();
const PANCAKE_ROUTER = ethers.getAddress(reqEnv("PANCAKE_ROUTER")).toLowerCase();
...
const EARLY_SCAN_BLOCKS = Number(process.env.EARLY_SCAN_BLOCKS || 600);

const MIN_BUYERS_EOA = Number(process.env.MIN_BUYERS_EOA || 13);
const MIN_SELLERS_EOA = Number(process.env.MIN_SELLERS_EOA || 4);
    ...
    try {
      ...
      if (!logs.length) {
        return {
          ...
          metrics: { realBuyers: 0, realSellers: 0 }
        };
      }
      ...
      for (const l of slice) {
        ...
        // Buy: tokens leaving pair to user
        if (from === pair) {
          buyers.set(to, (buyers.get(to) || 0n) + amt);
          buyBlocks.add(l.blockNumber);
        }

        // Sell: tokens coming from user back to pair
        if (to === pair) {
      ...
      const reasons = [];
      if (realBuyers.length < MIN_BUYERS_EOA) reasons.push("LOW_EOA_BUYERS");
      ...
      if (buyBlocks.size < Math.min(5, realBuyers.length)) reasons.push("BUY_CLUSTERED");

      // Holder concentration (top10) gate
      let holdersOk = false;
      try {
      ...
      // Simple score for debugging/telemetry
      let score = 0;
