...
export async function marketHealthPass(pairAddress) {
  for (let i = 0; i < RPC_URLS.length; i++) {
    try {
      ...
      const all = [...new Set([...buyers.keys(), ...sellers.keys()])];
      await batchIsEOA(all);
...
export async function swapFeeCheck(pairAddress, maxFeePct = 10) {
  for (let i = 0; i < RPC_URLS.length; i++) {
    try {
      ...
      const actualOut = await queue.add(() =>
        callProvider(p =>
          router.connect(p).getAmountsOut(amountIn, [WBNB, token]).then(r => r[1])
        )
      );
      ...
      console.log(`💰 Swap fee: ${feePct.toFixed(2)}%`);
