...
const MAX_BLOCK_BATCH = 5; // Alchemy safe batch size
...
async function getBNBLiquidity(pairAddress, tokenMint) {
  return withRpcFailover(async (prov) => {
    ...
    const bnbReserve = token0.toLowerCase() === tokenMint.toLowerCase() ? reserves.reserve1 : reserves.reserve0;
...
function savePair(entry) {
  ...
  if (data.some(p => p.pairaddress.toLowerCase() === entry.pairaddress.toLowerCase())) return;

  // Force checksummed addresses
...
async function scan() {
  ...
  // Batch blocks to avoid RPC errors
  for (let from = lastBlock + 1; from <= currentBlock; from += MAX_BLOCK_BATCH) {
    ...
    const logs = await withRpcFailover(p =>
      p.getLogs({ address: PANCAKE_FACTORY, fromBlock: from, toBlock: to, topics: [PAIR_CREATED_TOPIC] })
    );
