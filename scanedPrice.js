...
const ADDRESSES = {
  WBNB: ethers.getAddress(String(process.env.WBNB_ADDRESS).trim()),
  FACTORY: ethers.getAddress(String(process.env.PANCAKE_FACTORY).trim()),
  ROUTER: ethers.getAddress(String(process.env.PANCAKE_ROUTER).trim()),
};
 ...
 *
 * Extras kept from your old version:
 * - priceUSD, liquidityBNB, marketCap (BNB), bnbReserve
...
export async function scanedPrice(tokenMintRaw, pairAddressRaw = null, opts = {}) {
  ...
  if (pairAddressRaw) {
    try {
      ...
      pairOverride = null;
      if (debug) console.log("[scanedPrice] invalid pair override:", pairAddressRaw);
    }
  }

  // Load JSON only as a non-blocking cache hook
  ...
  const out = await withRpcFailover(async (prov) => {
    ...
    return {
      ...
      liquidityBNB,
      liquidityUSD,
      marketCap: marketCapBNB,          // keep your old key name
      marketCapBNB,                      // also available if you want it
      bnbReserve: wbnbReserve,           // keep your old key name
      totalSupply,
      tokenDecimals,
