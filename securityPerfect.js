...
const MIN_LIQUIDITY_BNB = Number(process.env.MIN_LIQUIDITY_BNB || 40);
...
function opcodeRiskSignals(bytecodeHex) {
  ...
  let risk = 0;
  if (minimalProxy) return { minimalProxy, hasDelegatecall, hasSelfdestruct, hasCreate2, risk: 1 };

  if (hasSelfdestruct) risk += 0.5;
  if (hasDelegatecall) risk += 0.25;
  if (hasCreate2) risk += 0.15;

  return { minimalProxy, hasDelegatecall, hasSelfdestruct, hasCreate2, risk: clamp(risk, 0, 1) };
}
...
export async function securityPerfect(pair) {
  ...
  // Bytecode fetch (also used for veto + strings)
  ...
  if (op.minimalProxy || op.risk >= 0.85) {
    return {
      ok: true,
      pass: false,
      reason: "VETO_PROXY_OR_HIGH_OPCODE_RISK",
      pair: p,
      token,
      score: 0,
      minScore: MIN_SECURITY_SCORE,
      breakdown: { opcodeRisk: op }
    };
  }
