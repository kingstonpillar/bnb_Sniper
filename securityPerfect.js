...
/* ================= FREEZE AUTHORITY (MAX 20) =================
   Goal: Detect admin capability to freeze/blacklist/pause transfers.
   Score meaning: 20 = best (no freeze controls detected)
*/
async function freezeAuthorityScore20(tokenAddr, bytecodeHex) {
  ...
  // helper: check if a function exists by calling it (view) or probing (non-view may revert)
  async function probeFn(abiFrag, args = []) {
    try {
      ...
      return { exists: true, value: v ?? null };
    } catch {
  ...
  const kwHits =
    ...
    (codeLower.includes("transferdelay") ? 1 : 0) +
    (codeLower.includes("cooldown") ? 1 : 0);

  // Pause signals
  const paused = await probeFn("function paused() view returns (bool)");
  const hasPause = (await probeFn("function pause()")).exists;
  const hasUnpause = (await probeFn("function unpause()")).exists;

  // Blacklist/freeze view signals
  const probeAddr = ZERO; // or DEAD

  const isBlacklisted = (await probeFn("function isBlacklisted(address) view returns (bool)", [probeAddr])).exists;
  const blacklisted   = (await probeFn("function blacklisted(address) view returns (bool)", [probeAddr])).exists;
  const isFrozen      = (await probeFn("function isFrozen(address) view returns (bool)", [probeAddr])).exists;
  const frozen        = (await probeFn("function frozen(address) view returns (bool)", [probeAddr])).exists;

  // Setter signals (presence check; calls may revert, but existence still matters)
  const setBlacklist =
    (await probeFn("function blacklist(address,bool)")).exists ||
    (await probeFn("function setBlacklist(address,bool)")).exists ||
    (await probeFn("function setBlackList(address,bool)")).exists;

  const setFreeze =
    (await probeFn("function freeze(address,bool)")).exists ||
    (await probeFn("function setFreeze(address,bool)")).exists;

  const flags = {
    pausedFn: paused.exists,
    pausedValue: paused.exists ? Boolean(paused.value) : null,
    pauseFns: Boolean(hasPause || hasUnpause),
    blacklistViews: Boolean(isBlacklisted || blacklisted || isFrozen || frozen),
    blacklistSetters: Boolean(setBlacklist || setFreeze),
    keywordHits: kwHits
  };

  // Score out of 20 (higher safer)
  let score = 20;

  // pause capability is a strong centralized control
  if (flags.pausedFn || flags.pauseFns) score -= 10;
  if (flags.pausedValue === true) score -= 8;

  // blacklist/freeze capability is also strong
  if (flags.blacklistViews) score -= 10;
  if (flags.blacklistSetters) score -= 6;

  // small heuristic penalty
  if (flags.keywordHits >= 6) score -= 2;
  else if (flags.keywordHits >= 3) score -= 1;

  score = clamp(score, 0, 20);

  return { score, flags, reason: "OK" };
}
