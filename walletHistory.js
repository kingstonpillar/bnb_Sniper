...
export async function checkLpOwner(token) {
  ...
  if (!pairAddr || pairAddr === ZERO_ADDR) {
    return { 
      lpExists: false, 
      totalScore: 0n, 
      metrics: { lpBurnedPercent: 0n, devLpPercent: 0n, lockedPercent: 0n }, 
      rug: null 
    };
  }
  ...
  // 3️⃣ Total supply (BigInt)
  const totalSupplyRaw = await lp.methods.totalSupply().call();
  ...
  // 4️⃣ LP balances
  const burnBalanceRaw0 = await lp.methods.balanceOf(ZERO_ADDR).call();
  const burnBalanceRaw1 = await lp.methods.balanceOf("0x000000000000000000000000000000000000dEaD").call();
  const burnBalance = BigInt(burnBalanceRaw0 || "0") + BigInt(burnBalanceRaw1 || "0");
  const devBalance = deployer 
    ? BigInt(await lp.methods.balanceOf(deployer).call() || "0") 
    : 0n;
...
export async function forkSellSimulation(token) {
  ...
  const ethIn = ethers.parseEther("0.01"); // already BigInt
  ...
  const tokenReserve = token0 === token.toLowerCase()
    ? BigInt(reserves[0])
    : BigInt(reserves[1]);

  const wbnbReserve = token0 === token.toLowerCase()
    ? BigInt(reserves[1])
    : BigInt(reserves[0]);
  ...
  // ---------- BUY TAX ----------
  const expectedBuyOut = (ethIn * tokenReserve * 9975n) /
                         (wbnbReserve * 10000n + ethIn * 9975n);
  ...
  if (buyTaxPercent > 10n) {
    return {
      ok:false,
      honeypot:true,
      reason:`HIGH_BUY_TAX_${buyTaxPercent}%`
    };
  }
  ...
  const expectedOut = (tokenBalance * wbnbReserve * 9975n) /
...
export async function walletRate(token) {
  ...
  if (lp.lpExists) {
    const lpScore = lp.totalScore > 125n ? 125n : BigInt(lp.totalScore || 0n);
  ...
  const feePercentBI = BigInt(fee?.priceImpactPercent || 0n); // wrap in BigInt
  if (fee.ok && feePercentBI <= 10n) {
    result.details.swapFee = {
      score: 15n,
      status: "healthy",
      feePercent: feePercentBI
    };
  ...
  // ✅ Health based on total score threshold (BigInt comparison)
