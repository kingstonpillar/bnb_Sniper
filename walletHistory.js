...
export async function safeRpcCall(fn, maxRetries = RPCS.length) {
  let lastError;

  for (let attempt = 0; attempt < maxRetries; attempt++) {
    const web3 = getWeb3();
    try {
      // Wrap in queue to limit concurrent RPC requests
      return await queue.add(() => fn(web3));
    } catch (err) {
      lastError = err;
      rotateRpc(); // try next RPC
    }
  }

  console.error("All RPCs failed:", lastError);
 ...
 */
 async function detectSellRestrictionStrict(token, provider) {
  // 1️⃣ Check bytecode
  ...
  for (const slot of storageSlots) {
    const value = await provider.getStorageAt(token, slot);
    if (value && value !== "0x0") {
...
/* =========================================================
  ...
  return safeRpcCall(async (web3) => {
    ...
    try {
      const amount0 = 1;
      const amount1 = 1;
      await pairContract.methods.swap(amount0, amount1, dummy, "0x").call({ from: dummy });
    } catch {
    ...
    try {
      ...
      flags.syncBlocked = false;
    }

    const blocked = flags.swapBlocked || flags.noReserves;
...
export async function walletRate(token) {
  ...
  return detectSellRestrictionStrict(token, provider);
});
