...
/* ===========================================================
    WALLET RATE — STATIC ANTI-SELL + TAX CHECK + RPC FAILOVER
=========================================================== */
...
// Safe provider call with auto RPC rotation
...
async function antiSellScan(token) {
  ...
  if (!token) return { ok: false, flags: ["INVALID_ADDRESS"] };

  const bytecode = await safeProviderCall(p => p.getCode(token));
  ...
  const hard = flags.filter(f =>
    ["TX_ORIGIN_USED", "TRANSFER_REVERT_LOGIC", "PAIR_REFERENCED"].includes(f)
  );
...
async function staticTaxCheck(token, tokenIn, factoryAddr) {
  ...
  factoryAddr = normalizeAddress(factoryAddr);

  if (!token || !tokenIn || !factoryAddr) return { ok: false, reason: "INVALID_ADDRESS" };

  const factory = new ethers.Contract(factoryAddr, [
    "function getPair(address,address) view returns(address)"
  ], provider);

  const pairAddress = await safeProviderCall(p => factory.getPair(token, tokenIn));
  ...
  const token0 = normalizeAddress(await safeProviderCall(p => pair.token0()));

  const tokenReserve = BI(token0 === token ? reserves.reserve0 : reserves.reserve1);
  const tokenInReserve = BI(token0 === token ? reserves.reserve1 : reserves.reserve0);

  if (tokenReserve === 0n || tokenInReserve === 0n) return { ok: false, reason: "EMPTY_LP" };

  // Static buy & sell tax calculation
  const ethIn = 10n ** 16n; // 0.01 tokenIn
...
// ---------------- WALLET RATE ----------------
export async function walletRate(token, tokenIn, factoryAddr) {
  const details = {};
  let score = 0;

  // Fill defaults from .env if missing
  tokenIn = tokenIn || process.env.WBNB_ADDRESS;
  factoryAddr = factoryAddr || process.env.PANCAKE_FACTORY;

  // Normalize addresses
  try {
    token = ethers.getAddress(token?.trim());
    tokenIn = ethers.getAddress(tokenIn?.trim());
    factoryAddr = ethers.getAddress(factoryAddr?.trim());
  } catch {
    return { token, totalScore: "0", health: "unhealthy", details: { reason: "INVALID_ADDRESS" } };
  }

  // Anti-sell scan
  const scan = await antiSellScan(token);
  if (!scan.ok) {
    details.antiSell = { score: 0, status: "unhealthy", flags: scan.flags };
    return { token, totalScore: "0", health: "unhealthy", details };
  }
  details.antiSell = { score: 40, status: "healthy", flags: scan.flags };
  score += 40;

  // Static buy/sell tax check
  const taxCheck = await staticTaxCheck(token, tokenIn, factoryAddr);
  if (!taxCheck.ok) {
    details.taxCheck = { score: 0, status: "unhealthy", reason: taxCheck.reason };
    return { token, totalScore: "40", health: "unhealthy", details };
  }
  details.taxCheck = { score: 40, status: "healthy", buyTaxPercent: taxCheck.buyTaxPercent, sellTaxPercent: taxCheck.sellTaxPercent };
  score += 40;
