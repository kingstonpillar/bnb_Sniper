...
export async function staticTaxCheck(pairAddress) {
  ...
  const sellTaxPercent = biPct(
    biSub(expectedSellNoTax, expectedSellOut),
    expectedSellNoTax
  );
  ...
  return {
    ...
    sellTaxPercent: Number(sellTaxPercent)
  };
}
...
export async function walletRate(pairAddress) {
  ...
  if (!details.taxCheck.ok && !details.taxCheck.reason)
    details.taxCheck.reason = ["TAX_CHECK_FAILED"];

  // ---------------- LP OWNERSHIP ----------------
  const lpOwnerRaw = await lpOwnershipScore(pairAddress);
  details.lpOwner = {
    ok: lpOwnerRaw.ok,
  ...
  if (token && (await isProjectContract(token, provider))) {
    projectScore = 25; // fixed score for project contracts
  ...
  // ---------------- ROUTER PENALTY ----------------
  // Assuming the pairAddress uses a router (for LP, usually PancakeSwap)
  let routerPenalty = 0;
  let routerFlagged = false;

  if (token && token !== WBNB) {
    const scan = await scanRouter(token, provider);
    if (!scan.ok) {
      routerPenalty = 50; // fixed penalty for malicious router
      routerFlagged = true;
  ...
  // ---------------- TOTAL SCORE & HEALTH ----------------
  // Health score = 120 max (all good) + project 25 = 145 (or 150 with your previous base)
  const allOk =
    details.antiSell.ok &&
    details.taxCheck.ok &&
    details.lpOwner.ok &&
  ...
  const totalScore = baseScore + projectScore - routerPenalty; // subtract router penalty if flagged
