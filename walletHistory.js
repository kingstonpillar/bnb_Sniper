...
/* ================= SELL SIMULATION ================= */
async function simulateRealSell(token) {
  return queue.add(() =>
    safeCall(async (web3) => {
      const dev = await getDevWallet(token);
      if (!dev) return { sellable: false, reason: "NO_DEV_WALLET" };

      // ✅ ensure dev actually owns tokens
      const erc20 = new web3.eth.Contract(ERC20_ABI, token);
      const bal = await erc20.methods.balanceOf(dev).call();

      const AMOUNT_IN = 1000000000000000n; // dust amount

      if (BigInt(bal) < AMOUNT_IN) {
        return { sellable: false, reason: "DEV_HAS_NO_TOKENS" };
      }
...
export async function walletRate(token) {
  ...
  // 1️⃣ SELL CHECK — 20 POINTS
  const sell = await simulateRealSell(token);
  if (sell.sellable) {
    result.details.sellSimulation = { score: 20, status: "healthy" };
    result.totalScore += 20;
  }

  // 2️⃣ LP CONTROL — 15 POINTS
  ...
  // 3️⃣ DEV WALLET REPUTATION — 10 POINTS
  const dev = await getDevWallet(token);
  if (dev) {
