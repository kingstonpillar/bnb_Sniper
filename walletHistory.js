...
const WBNB = safeNormalize(process.env.WBNB_ADDRESS);
if (!WBNB) throw new Error("WBNB_ADDRESS not defined in .env");
...
export async function staticTaxCheck(pairAddress) {
  ...
  try {
    ({ reserves, token0, token1 } = await safeCall(async (prov) => {
      const pair = new ethers.Contract(pAddr, ERC20_ABI, prov);
      return {
...
export async function walletRate(pairAddress) {
  ...
  if (!details.antiSell.ok) details.antiSell.reason = details.antiSell.flags?.length ? details.antiSell.flags : ["ANTI_SELL_DETECTED"];
