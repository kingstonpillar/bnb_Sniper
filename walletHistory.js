...
export async function checkLpOwner(token) {
  ...
  return {
    ...
    rug,
    totalScore: (lpScore + rugScore).toString()
  };
}
/* ===========================================================
...
export async function swapFeeCheck(token) {
  ...
  const tokenReserve = token0 === token.toLowerCase() ? BigInt(reservesRaw[0].toString()) : BigInt(reservesRaw[1].toString());
  ...
  return { ok:true, priceImpactPercent: priceImpactPercent.toString() };
/* ===========================================================
...
export async function walletRate(token) {
  ...
  if (lp.lpExists) {
    const lpScore = lp.totalScore > 125n ? 125n : BigInt(lp.totalScore);
    const lpStatus = lpScore >= 100n ? "healthy" : "unhealthy";
    result.details.lpOwnership = {
      score: lpScore.toString(),
      status: lpStatus,
      metrics: {
        ...lp.metrics,
        lpBurnedPercent: lp.metrics.lpBurnedPercent.toString(),
        devLpPercent: lp.metrics.devLpPercent.toString(),
        lockedPercent: lp.metrics.lockedPercent.toString(),
        lockExpiry: lp.metrics.lockExpiry?.toString()
      },
      rug: lp.rug
    };
    result.totalScore = (BigInt(result.totalScore) + lpScore).toString();
  }

  const sell = await forkSellSimulation(token);
  if (sell.ok) {
    result.details.sellSimulation = { score: "20", status: "healthy", honeypot: !!sell.honeypot };
    result.totalScore = (BigInt(result.totalScore) + 20n).toString();
  } else {
  ...
  if (fee.ok && feePercentBI <= 10n) {
    result.details.swapFee = { score: "15", status: "healthy", feePercent: feePercentBI.toString() };
    result.totalScore = (BigInt(result.totalScore) + 15n).toString();
  }
  ...
  result.health = BigInt(result.totalScore) >= 130n ? "healthy" : "unhealthy";
  return result;
}