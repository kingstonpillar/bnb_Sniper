...
export async function walletRate(pairAddress) {
  ...
  // ---------------- TAX CHECK ----------------
  details.taxCheck = await safeCall(() => staticTaxCheck(pairAddress));
  ...
  if (token && await safeCall(() => isProjectContract(token, provider))) {
  ...
  if (token) {
    const scan = await safeCall(() => scanRouter(token, provider));
    if (!scan.ok) {
