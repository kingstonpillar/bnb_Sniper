...
// ----------------- load addresses -----------------
let WBNB, ROUTER, FACTORY;

try {
  WBNB = loadAddr("WBNB_ADDRESS");
  ROUTER = loadAddr("PANCAKE_ROUTER");
  FACTORY = loadAddr("PANCAKE_FACTORY");
  console.log("Addresses loaded:", { WBNB, ROUTER, FACTORY });
} catch (e) {
  console.error("Error loading addresses:", e.message);
  process.exit(1); // stop execution if something is wrong
}
...
export async function checkLpOwner(token) {
  ...
  return safeRpcCall(async (web3) => {
    ...
    // ---------- DEV MEMORY CACHE ----------
    if (!devMemory[token]) {
      devMemory[token] = { rugs: 0, dev: null, rug: null };
    }

    // ---------- DEV WALLET ----------
    if (!devMemory[token].dev) {
      devMemory[token].dev = await getDevWallet(token);
    }
    ...
    // ---------- RUG PENALTY ----------
    if (!devMemory[token].rug) {
      devMemory[token].rug = await delayedRugCheck(token, FORK_RPC);
    }
