...
export async function detectSellRestrictions(token) {
  return queue.add(() =>
    safeCall(async (web3) => {
      ...
      // 1️⃣ Get LP pair with WBNB
      const pair = await new web3.eth.Contract(FACTORY_ABI, FACTORY)
        .methods.getPair(token, WBNB)
        .call();
      ...
      let flags = {
        transferBlocked: false,      // dummy wallet fails
        transferFromBlocked: false,  // ignored for normal sell
        pairBlocked: false,          // pair cannot send tokens
        routerBlocked: false         // router cannot send tokens
      };

      // 2️⃣ Test transfer() from dummy wallet to pair
      try {
      ...
      // 6️⃣ Final decision
      let blocked =
        flags.pairBlocked || flags.routerBlocked || (flags.transferBlocked && flags.routerBlocked);

      // Optionally log dummy wallet issues without penalizing
      if (!blocked && flags.transferBlocked) {
        console.log(`⚠️ Dummy wallet transfer failed for ${token}, but router works.`);
      }
...
export async function walletRate(token) {
  ...
  if (sell.blocked) {
    // True sell restriction → unhealthy
    ...
    // Router works → healthy
    let score = 20;
    let status = "healthy";

    if (sell.flags.transferBlocked) {
      // Dummy wallet failed, log warning but do not penalize
      console.log(`⚠️ Dummy wallet transfer failed for ${token}, but router works.`);
    }
