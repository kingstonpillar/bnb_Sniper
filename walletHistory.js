...
export async function checkLpOwner(token) {
  ...
  const lockLongEnough =
    lockExpiry && lockExpiry - BigInt(Math.floor(Date.now() / 1000)) >= 30n * 24n * 3600n;
...
export async function walletRate(token) {
  ...
  if (lp.lpExists) {
    result.details.lpOwnership = {
      score: BigInt(lp.totalScore > 125 ? 125 : lp.totalScore),
      status: BigInt(lp.totalScore) >= 100n ? "healthy" : "unhealthy",
      metrics: lp.metrics,
      rug: lp.rug
    };
