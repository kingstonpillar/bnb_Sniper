...
export const ERC20_ABI = [
  ...
  // ---------------- Mint / Role Control ----------------
  "function mint(uint256 amount) returns (bool)",          // Mint tokens
  "function hasRole(bytes32 role, address account) view returns (bool)", // Check roles
  "function MINTER_ROLE() view returns (bytes32)",         // Standard minter role variable
  "function owner() view returns (address)",              // Ownership check (if Ownable)
  
  // ---------------- Optional Safety / Pausable ----------------
  "function paused() view returns (bool)",                // If contract supports Pausable
  "function isExcludedFromFee(address account) view returns (bool)", // Common tax exemptions
  "function isBlacklisted(address account) view returns (bool)"     // Anti-scam / blacklist check
];
...
// ---------------- LP OWNERSHIP ----------------
export async function lpOwnershipScore(pairAddress, provider) {
  try {
    const lp = new ethers.Contract(pairAddress, LP_ABI, provider);

    // Fetch LP totals and balances safely
    const [totalLPRaw, burnedLPRaw] = await Promise.all([
      lp.totalSupply().catch(() => 0n),
      lp.balanceOf(BURN_ADDRESS).catch(() => 0n)
    ]);

    const totalLP = BigInt(totalLPRaw);
    const burnedLP = BigInt(burnedLPRaw);

    let lockedLP = 0n;

    // Sum locked LP in known lockers
    for (const locker of KNOWN_LOCKERS) {
      try {
        const bal = await lp.balanceOf(locker);
        lockedLP += BigInt(bal);
      } catch {}
    }

    // Prevent lockedLP from exceeding totalLP
    if (lockedLP > totalLP) lockedLP = totalLP;

    // Percentages
    const burnPercent = totalLP > 0n ? Number((burnedLP * 10000n) / totalLP) / 100 : 0;
    const lockPercent = totalLP > 0n ? Number((lockedLP * 10000n) / totalLP) / 100 : 0;

    // LP is OK if either >95%
    const ok = burnPercent >= 95 || lockPercent >= 95;
    const reason = ok ? [] : ["LP not sufficiently locked or burned"];
    ...
    return {
      ok: false,
      reason: ["LP_CHECK_FAILED", err.message],
      raw: {}
    };
  }
}
...
export async function staticTaxCheck(pairAddress) {
  ...
  const data = await safeCall(async (provider) => {
    const pair = new ethers.Contract(pAddr, LP_ABI, provider);
