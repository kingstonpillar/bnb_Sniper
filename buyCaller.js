...
function trendConfirmed(candles) {
  ...
  // Structure-only confirmation: Higher High + Higher Low
...
function reverseDetected(candles) {
  ...
  // Immediate structure break: Lower Low versus previous candle
  if (last.low < prev.low) return true;

  // Optional: 3-candle rolling lower highs pattern
  if (candles.length >= 3) {
    const prev2 = candles[candles.length - 3];
    if (last.high < prev.high && prev.high < prev2.high) return true;
  }
...
function updateTrendPersistence(token, candles) {
  ...
  // Initialize only if missing (do not reset when 0)
  if (token.trendPersistence == null) token.trendPersistence = 0;

  if (trendConfirmed(candles)) token.trendPersistence += 1;
...
const isMarketHealthy = !!marketPass?.isHealthy;
const isLpSafe = !!safetyResult?.lpStatus?.safe;


if (
  (token.trendPersistence || 0) >= REQUIRED_PERSISTENCE &&
