...
function reverseDetected(candles, rise) {
  ...
  const last = candles[candles.length - 1];
  const prev = candles[candles.length - 2];
  // Fake pump/dump or distribution
...
function updateTrendPersistence(token, candles) {
  ...
  if (trendConfirmed(candles, rise)) {
    token.trendPersistence += 1;
  } else {
...
async function processToken(token) {
  ...
  try {
    ...
    if (!token.entryPrice) {
      ...
      token.entryTime = now;
      token.deleteTime = now + 10 * 60 * 1000; // 10 min tracking
      token.candles = [{ open: priceNow, close: priceNow, high: priceNow, low: priceNow, startTime: now }];
      return;
    }
    ...
    candles.push({ open: candles[candles.length-1].close, close: priceNow, high: Math.max(priceNow, candles[candles.length-1].close), low: Math.min(priceNow, candles[candles.length-1].close), startTime: now });
    if (candles.length > 3) candles.shift(); // keep only last 3 candles
    token.candles = candles;

    // -------------------- REVERSE DETECTION --------------------
    if (reverseDetected(candles, rise)) {
      console.log(`🔻 ${tokenMint} reverse detected, removing`);
      token.remove = true;
      return;
    }
    ...
    if (token.trendPersistence >= REQUIRED_PERSISTENCE) {
      console.log(`🚀 BUY ${tokenMint} @ ${priceNow} BNB (trend confirmed ${token.trendPersistence}x)`);
    ...
    console.error(`⚠️ ${tokenMint} error:`, err.message);
  }
}
