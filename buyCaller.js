...
async function processToken(token) {
  ...
  try {
    ...
    // Run market health pass dynamically
const marketPass = await marketHealthPass(pairAddress);
    ...
    // Only proceed if dynamic market health is ok
if (!marketPass || scan.riskScore >= 4 || velocityTooHigh || mcapLiquidityBad || fakeFlowDetected) {
  token.remove = true;
  return;
}
    // -------------------- UPDATE CANDLES --------------------
    ...
    console.error(`Error processing ${tokenMint}:`, err.message);
  }
}
