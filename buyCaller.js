...
const OBSERVE_MINUTES = 1; // 60 seconds
...
async function processToken(token) {
  ...
  if (token.deleteTime && now >= token.deleteTime) {
    console.log("[processToken] REMOVE: expired", {
      now,
      deleteTime: token.deleteTime
    });
    removeToken();
    return;
  }
  ...
  if (!token.staticChecked) {
    try {
      ...
      if (!token.staticChecked) {
        console.log("[processToken] REMOVE: static failed", {
          secPerfectPass,
          secSafetyPass,
          liqLockedPass,
          walletPass
      ...
      if (!Number.isFinite(entryPx) || entryPx <= 0) {
        console.log("[processToken] REMOVE: bad entry price", { entryPx });
        removeToken();
        return;
      }
      ...
      console.log("[processToken] REMOVE: static threw error", {
  ...
  if (token.staticChecked === true && token.complimentaryChecked !== true) {
    try {
      ...
      console.log("[processToken] REMOVE: complimentary threw error", {
        err: e?.message || String(e)
      });
      removeToken();
      return;
  ...
  if (!Number.isFinite(priceNow) || priceNow <= 0) {
    console.log("[processToken] REMOVE: bad priceNow", { priceNow });
    removeToken();
    return;
  }
  if (!Number.isFinite(token.entryPrice) || token.entryPrice <= 0) {
    console.log("[processToken] REMOVE: missing entryPrice", { entryPrice: token.entryPrice });
    removeToken();
    return;
  }
  ...
  if (safetyResult?.safeToBuy !== true) {
    console.log("[processToken] WAIT: safeToBuy false", {
      ...
      market: safetyResult?.marketBehavior || safetyResult?.marketHealth
    });
    return;
  }
  ...
  if (!marketPass?.isHealthy) {
    console.log("[processToken] WAIT: market not healthy", { marketPass });
    return;
  }
