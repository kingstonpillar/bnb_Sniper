...
if (!token.staticChecked) {
  try {
    const [secPerfect, secSafety, liqLocked, walletRes] = await Promise.all([
      ...
      walletRate(tokenMint, WBNB_ADDRESS, PANCAKE_FACTORY) // <-- pass all required args
    ]);

    // walletRate returns an object, so access health directly
    const walletHealthy = walletRes.health === "healthy";

    // Example: mark staticChecked if all security checks passed
    token.staticChecked = secPerfect && secSafety && liqLocked && walletHealthy;

  } catch (err) {
    console.error("Static security check failed:", err.message);
