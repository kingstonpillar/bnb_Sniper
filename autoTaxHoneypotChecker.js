...
export async function autoTaxHoneypotCheck(pairAddress) {
  ...
  // Honeypot pass: must simulate successfully + not honeypot.
  // Taxes can be 0, so do NOT require non-null only from the wrong object.
  const safeByHp =
    hp.success &&
    hp.isHoneypot === false &&
    hp.sellTax !== null; // keep strict: requires we got a numeric tax (0 is valid)

  // GoPlus pass: strict. If GoPlus fails, safeByGp becomes false.
