...
// BSC WBNB (checksummed + normalized)
...
// GoPlus Labs Free Token Security API (requires API key)
...
export async function autoTaxHoneypotCheck(pairAddress) {
  ...
  const [hp, gp] = await Promise.all([checkHoneypotIs(token), checkGoPlusSecurity(token)]);

  const safeByHp = hp.success && hp.sellTax !== null && hp.isHoneypot === false;
  const safeByGp =
    gp.error
      ? false
      : !gp.signals?.is_honeypot && !gp.signals?.cannot_sell_all && !gp.signals?.is_blacklisted;
