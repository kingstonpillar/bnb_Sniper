...
// your existing logic (must be above and named exactly sellCaller)
export async function sellCaller() {
  const positions = readPositions();

  if (!positions.length) {
    console.log("[sellCaller] no active positions");
    return;
  }

  console.log("[sellCaller] tick", { positions: positions.length });

  // run checks through the queue so we do not overload RPC
  const tasks = positions.map((pos) => sellQueue.add(() => checkPosition(pos)));

  await Promise.allSettled(tasks);
}

// ---------------- MONITOR LOOP START/STOP (PM2 + direct node friendly) ----------------
let sellTimer = null;
...
async function runSellTick(label) {
  ...
  try {
    await sellCaller(); // calls the exported function above
  } catch (err) {
...
export function startSellCaller() {
  ...
  const CHECK_INTERVAL_MS = Number(process.env.SELL_TICK_MS || 10_000);
  if (!Number.isFinite(CHECK_INTERVAL_MS) || CHECK_INTERVAL_MS <= 0) {
    throw new Error("Invalid SELL_TICK_MS env value");
  }
