...
export async function securitySafety(pairAddress, tokenMint) {
  ...
  // Early fail if basic checks fail
  if (!pairOk || bytecodeScore < 10) return false;

  // Initialize total score
  let score = 0;

  // Add remaining checks (without lpBurnScore)
  score += bytecodeScore;                  // 20
  // score += await lpBurnScore(pairAddress); // removed
  score += await devWalletScore(pairAddress);   // 10
  score += await ownershipScore(pairAddress);   // 5
  score += await sizeScore(pairAddress);        // 5
  score += await honeypotScore(pairAddress);    // 15
  score += await timeTrapScore(pairAddress);    // 10

  // Calculate maximum possible score without LP burn
  const maxScore = 20 + 10 + 5 + 5 + 15 + 10; // 65

  console.log(`🔐 FREE RPC security score for pair ${pairAddress}: ${score}/${maxScore}`);
  return score >= 70; // You may adjust threshold because maxScore < 70 now
}