...
export async function securitySafety(pairAddress, tokenMint) {
  ...
  score += await bytecodeHashSimilarityCheck(tokenMint); // 20
  score += await lpBurnScore(pairAddress);              // 20
  ...
  console.log(`🔐 FREE RPC security score for pair ${pairAddress}: ${score}/85`);
  return score >= 70;
}